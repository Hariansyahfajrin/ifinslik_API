...
namespace API.Controllers
{
	...
	public class MasterTemplateController(IMasterTemplateService service, IConfiguration configuration) : BaseController(configuration)
	{
		private readonly IMasterTemplateService _service = service;
		...
		[HttpPost("Insert")]
		public async Task<ActionResult> Insert(MasterTemplate id)
		{
			try
			{
		...
		[HttpPut("UpdateByID")]
		public async Task<ActionResult> UpdateByID(MasterTemplate category)
		{
			try
			{
        ...
        public async Task<ActionResult> ChangeStatus(MasterTemplate masterTemplate )
        {
            try
            {
				return ResponseSuccess(null, await _service.ChangeStatus(masterTemplate));
            }
            catch (Exception ex)
            {
                return ResponseError(ex);
