...
namespace DAL
{
	public class A01Repository : BaseRepository, IA01Repository
	{

		private readonly string tableBase = "a01";

		private readonly string tableCompany = "sys_company";

		public async Task<List<A01>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit, string formTransactionID)
		{
			var p = db.Symbol();
			...
			return await _command.GetRows<A01>(transaction, query, new
			{
				Keyword = $"%{keyword}%",
				Limit = limit,
				Offset = offset,
		...
		public async Task<A01> GetRowByID(IDbTransaction transaction, string id)
		{
			var p = db.Symbol();
			...
			var result = await _command.GetRow<A01>(
		...
		public async Task<int> Insert(IDbTransaction transaction, A01 model)
		{
			var p = db.Symbol();
		...
		public async Task<int> UpdateByID(IDbTransaction transaction, A01 model)
		{
			var p = db.Symbol();
		...
		public Task<List<A01>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit)
		{
