...
namespace DAL
{
	public class D01HistoryRepository : BaseRepository, ID01HistoryRepository
	{

		private readonly string tableBase = "d01_history";
		private readonly string tableCompany = "sys_company";

		public async Task<List<D01History>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit, string formTransactionHistoryID)
		{
			var p = db.Symbol();
			...
			string query = QueryLimitOffset(
					...
					select
								...
								,nik											as	Nik								
								,nama											as	Nama										
								,jenis_kelamin_desc								as	JenisKelamin_Desc							
			...
			return await _command.GetRows<D01History>(transaction, query, new
			{
				Keyword = $"%{keyword}%",
				Limit = limit,
				Offset = offset,
		...
		public async Task<D01History> GetRowByID(IDbTransaction transaction, string id)
		{
			var p = db.Symbol();
			...
			var result = await _command.GetRow<D01History>(
		...
		public async Task<int> Insert(IDbTransaction transaction, D01History model)
		{
			var p = db.Symbol();
		...
		public async Task<int> UpdateByID(IDbTransaction transaction, D01History model)
		{
			var p = db.Symbol();
		...
		public async Task<List<D01History>> GetRowsForLookup(IDbTransaction transaction, string keyword, int offset, int limit, string? period, string? financeCompanyType)
		{
			var p = db.Symbol();
        ...
        from
          (
            select
              d01_history.cif
              ,d01_history.nama_lengkap
              ,d01_history.period
              ,d01_history.mod_date
            from
              d01_history
              inner join form_transaction_history ftr on ftr.id = d01_history.form_transaction_history_id
            where
            ...
            select
              d02_history.cif
              ,d02_history.nama_badan_usaha
              ,d02_history.period
              ,d02_history.mod_date
            from
              d02_history
              inner join form_transaction_history ftr on ftr.id = d02_history.form_transaction_history_id
          ) as additional
        where
		...
		public Task<List<D01History>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit)
		{
