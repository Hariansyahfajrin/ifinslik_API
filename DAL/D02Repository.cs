...
namespace DAL
{
	public class D02Repository : BaseRepository, ID02Repository
	{

		private readonly string tableBase = "d02";
		private readonly string tableCompany = "sys_company";
		private readonly string tableForm = "form_transaction";

		public async Task<List<D02>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit, string formTransactionID)
		{
			var p = db.Symbol();
			...
			return await _command.GetRows<D02>(transaction, query, new
			{
				Keyword = $"%{keyword}%",
				Limit = limit,
				Offset = offset,
		...
		public async Task<D02> GetRowByID(IDbTransaction transaction, string id)
		{
			var p = db.Symbol();
			...
			var result = await _command.GetRow<D02>(
		...
		public async Task<int> Insert(IDbTransaction transaction, D02 model)
		{
			var p = db.Symbol();
		...
		public async Task<int> UpdateByID(IDbTransaction transaction, D02 model)
		{
			var p = db.Symbol();
		...
		public async Task<List<D02>> GetRowsForLookup(IDbTransaction transaction, string keyword, int offset, int limit, string? period, string? financeCompanyType)
		{
			var p = db.Symbol();
			...
			string query = QueryLimitOffset(
				...
				SELECT 
					...
					from 
						{tableBase} 
					INNER JOIN 
						{tableForm} ftr on ftr.id = {tableBase}.form_transaction_id
					WHERE 
						period = {p}Period
  					AND 
		...
		public Task<List<D02>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit)
		{
