...
namespace DAL
{
	public class F01HistoryRepository : BaseRepository, IF01HistoryRepository
	{

		private readonly string tableBase = "f01_history";
		...
		public async Task<List<F01History>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit, string formTransactionHistoryID)
		{
			var p = db.Symbol();
			...
			return await _command.GetRows<F01History>(transaction, query, new
			{
				Keyword = $"%{keyword}%",
				Limit = limit,
				Offset = offset,
		...
		public async Task<F01History> GetRowByID(IDbTransaction transaction, string id)
		{
			var p = db.Symbol();
			...
			var result = await _command.GetRow<F01History>(
		...
		public async Task<int> Insert(IDbTransaction transaction, F01History model)
		{
			var p = db.Symbol();
		...
		public async Task<int> UpdateByID(IDbTransaction transaction, F01History model)
		{
			var p = db.Symbol();
		...
		public async Task<List<F01History>> GetRowsForLookup(IDbTransaction transaction, string keyword, int offset, int limit, string? period, string? companyCode, string? financeCompanyType)
		{
			var p = db.Symbol();
            ...
            left join {tableD01} as d1 on d1.cif = {tableBase}.cif
            left join {tableD02} as d2 on d2.cif = {tableBase}.cif
            inner join {tableForm} as ftr on ftr.id = {tableBase}.form_transaction_history_id
            where
       ...
       "
		...
		public Task<List<F01History>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit)
		{
