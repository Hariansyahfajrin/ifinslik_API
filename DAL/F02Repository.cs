...
namespace DAL
{
	public class F02Repository : BaseRepository, IF02Repository
	{

		private readonly string tableBase = "f02";
		private readonly string tableD01 = "d01";
		private readonly string tableD02 = "d02";
		private readonly string tableCompany = "sys_company";

		public async Task<List<F02>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit, string formTransactionID)
		{
			var p = db.Symbol();
			...
			return await _command.GetRows<F02>(transaction, query, new
			{
				Keyword = $"%{keyword}%",
				Limit = limit,
				Offset = offset,
		...
		public async Task<F02> GetRowByID(IDbTransaction transaction, string id)
		{
			var p = db.Symbol();
			...
			var result = await _command.GetRow<F02>(
		...
		public async Task<int> Insert(IDbTransaction transaction, F02 model)
		{
			var p = db.Symbol();
		...
		public async Task<int> UpdateByID(IDbTransaction transaction, F02 model)
		{
			var p = db.Symbol();
		...
		public Task<List<F02>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit)
		{
