...
namespace DAL
{
	public class F06Repository : BaseRepository, IF06Repository
	{

		private readonly string tableBase = "f06";
		private readonly string tableCompany = "sys_company";
		private readonly string tableD01 = "d01";
		private readonly string tableD02 = "d02";

		public async Task<List<F06>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit, string formTransactionID)
		{
			var p = db.Symbol();
			...
			return await _command.GetRows<F06>(transaction, query, new
			{
				Keyword = $"%{keyword}%",
				Limit = limit,
				Offset = offset,
		...
		public async Task<F06> GetRowByID(IDbTransaction transaction, string id)
		{
			var p = db.Symbol();
			...
			var result = await _command.GetRow<F06>(
		...
		public async Task<int> Insert(IDbTransaction transaction, F06 model)
		{
			var p = db.Symbol();
		...
		public async Task<int> UpdateByID(IDbTransaction transaction, F06 model)
		{
			var p = db.Symbol();
		...
		public Task<List<F06>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit)
		{
