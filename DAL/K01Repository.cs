...
namespace DAL
{
	public class K01Repository : BaseRepository, IK01Repository
	{

		private readonly string tableBase = "k01";
		private readonly string tableCompany = "sys_company";
		private readonly string tableD01 = "d01";
		private readonly string tableD02 = "d02";

		public async Task<List<K01>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit, string formTransactionID)
		{
			var p = db.Symbol();
			...
			return await _command.GetRows<K01>(transaction, query, new
			{
				Keyword = $"%{keyword}%",
				Limit = limit,
				Offset = offset,
		...
		public async Task<K01> GetRowByID(IDbTransaction transaction, string id)
		{
			var p = db.Symbol();
			...
			var result = await _command.GetRow<K01>(
		...
		public async Task<int> Insert(IDbTransaction transaction, K01 model)
		{
			var p = db.Symbol();
		...
		public async Task<int> UpdateByID(IDbTransaction transaction, K01 model)
		{
			var p = db.Symbol();
		...
		public Task<List<K01>> GetRows(IDbTransaction transaction, string keyword, int offset, int limit)
		{
