...
namespace Service
{
  ...
  public class D02Service(ID02Repository repo, IFormTransactionRepository transactionRepo) : BaseService, ID02Service
  {
    private readonly ID02Repository _repo = repo;
    private readonly IFormTransactionRepository _transactionRepo = transactionRepo;

    public async Task<List<D02>> GetRows(string keyword, int offset, int limit, string formTransactionID)
    {
    ...
    public async Task<D02> GetRowByID(string id)
    {
      using var connection = _repo.GetDbConnection();
      using var transaction = connection.BeginTransaction();
    ...
    public async Task<int> Insert(D02 model)
    {
    ...
    public async Task<int> UpdateByID(D02 model)
    {
      using var connection = _repo.GetDbConnection();
      using var transaction = connection.BeginTransaction();
    ...
    public async Task<List<D02>> GetRowsForLookup(string keyword, int offset, int limit, string? formTransactionId)
    {
      using var connection = _repo.GetDbConnection();
      using var transaction = connection.BeginTransaction();
    ...
    public Task<List<D02>> GetRows(string keyword, int offset, int limit)
    {
