...
namespace Service
{
  ...
  public class F01Service(IF01Repository repo, IFormTransactionRepository transactionRepo) : BaseService, IF01Service
  {
    private readonly IF01Repository _repo = repo;
    private readonly IFormTransactionRepository _transactionRepo = transactionRepo;

    public async Task<List<F01>> GetRows(string keyword, int offset, int limit, string formTransactionID)
    {
    ...
    public async Task<F01> GetRowByID(string id)
    {
      using var connection = _repo.GetDbConnection();
      using var transaction = connection.BeginTransaction();
    ...
    public async Task<int> Insert(F01 model)
    {
    ...
    public async Task<int> UpdateByID(F01 model)
    {
      using var connection = _repo.GetDbConnection();
      using var transaction = connection.BeginTransaction();
    ...
    public async Task<List<F01>> GetRowsForLookup(string keyword, int offset, int limit, string? formTransactionId)
    {
      using var connection = _repo.GetDbConnection();
      using var transaction = connection.BeginTransaction();
    ...
    public Task<List<F01>> GetRows(string keyword, int offset, int limit)
    {
