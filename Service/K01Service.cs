...
namespace Service
{
  ...
  public class K01Service(IK01Repository repo, IFormTransactionRepository transactionRepo) : BaseService, IK01Service
  {
    private readonly IK01Repository _repo = repo;
    private readonly IFormTransactionRepository _transactionRepo = transactionRepo;

    public async Task<List<K01>> GetRows(string keyword, int offset, int limit, string formTransactionID)
    {
    ...
    public async Task<K01> GetRowByID(string id)
    {
      using var connection = _repo.GetDbConnection();
      using var transaction = connection.BeginTransaction();
    ...
    public async Task<int> Insert(K01 model)
    {
    ...
    public async Task<int> UpdateByID(K01 model)
    {
      using var connection = _repo.GetDbConnection();
      using var transaction = connection.BeginTransaction();
    ...
    public Task<List<K01>> GetRows(string keyword, int offset, int limit)
    {
